...
class OverviewTeamProfileViewController: HasTeamViewController {
    ...
    func loadCompetitions() {
        ...
        // Competitions
        let refComp = Firebase(url: "https://vexscout.firebaseio.com/teams/\(team.num.uppercaseString)/comps")
        refComp.observeSingleEventOfType(.ChildAdded, withBlock: { (snapshot:FDataSnapshot!) -> Void in
            ...
            while let rest = enumerator.nextObject() as? FDataSnapshot {
                ...
                // Matches
                let ref = Firebase(url: "https://vexscout.firebaseio.com/teams/\(self.team.num.uppercaseString)/comps/Skyrise/\(comp.name)/matches")
                ref.observeSingleEventOfType(.Value, withBlock: { (snapshot: FDataSnapshot!) -> Void in
                    ...
                    // Awards
                    let refAward = Firebase(url: "https://vexscout.firebaseio.com/teams/\(self.team.num.uppercaseString)/comps/Skyrise/\(comp.name)/awards")
